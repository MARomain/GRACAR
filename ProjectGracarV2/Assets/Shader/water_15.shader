...
Shader "Shader Forge/water_02" {
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _node_4630; uniform float4 _node_4630_ST;
...
////// Emissive:
                float4 node_3133 = _Time;
                float2 node_8744 = (i.uv0+node_3133.g*float2(0.05,0.05));
                float4 _NoiseG_var = tex2D(_NoiseG,TRANSFORM_TEX(node_8744, _NoiseG));
                float2 node_9779 = ((_NoiseG_var.g*0.1+0.0)+(i.uv0*0.5));
                float4 node_7601 = tex2D(_node_4630,TRANSFORM_TEX(node_9779, _node_4630));
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 node_7427 = _Time;
                float2 node_8744 = (i.uv0+node_7427.g*float2(0.05,0.05));
                float4 _NoiseG_var = tex2D(_NoiseG,TRANSFORM_TEX(node_8744, _NoiseG));
                float2 node_9779 = ((_NoiseG_var.g*0.1+0.0)+(i.uv0*0.5));
                float4 node_7601 = tex2D(_node_4630,TRANSFORM_TEX(node_9779, _node_4630));
                float3 Caustics = (((node_7601.rgb-(1.0 - __CausticsThickness))*__CausticsOpacity)*i.vertexColor.a);
