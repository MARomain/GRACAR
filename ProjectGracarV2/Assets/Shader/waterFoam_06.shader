...
////// Specular:
                ...
                float node_3908 = 0.0;
                float node_9087 = 1.0;
                float node_4197 = saturate((_FoamPower*saturate((node_3908 + ( (node_1661 - _FoamMin) * (node_9087 - node_3908) ) / (_FoamMax - _FoamMin)))));
                float3 diffuseColor = saturate((lerp(_ColorDeep.rgb,_ColorShallow.rgb,pow(1.0-max(0,dot(normalDirection, viewDirection)),clamp(_ColorFresnel,0,4)))+node_4197)); // Need this for specular when using metallic
...
/// Final Color:
                ...
                fixed4 finalRGBA = fixed4(lerp(sceneColor.rgb, finalColor,saturate(((node_4197.r*0.5)+(node_3908 + ( (node_1661 - _DepthMin) * (node_9087 - node_3908) ) / (_DepthMax - _DepthMin))))),1);
...
/// Final Color:
                ...
                fixed4 finalRGBA = fixed4(finalColor * saturate(((node_4197.r*0.5)+(node_3908 + ( (node_1661 - _DepthMin) * (node_9087 - node_3908) ) / (_DepthMax - _DepthMin)))),0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float node_9087 = 1.0;
                float node_4197 = saturate((_FoamPower*saturate((node_3908 + ( (node_1661 - _FoamMin) * (node_9087 - node_3908) ) / (_FoamMax - _FoamMin)))));
                float3 diffColor = saturate((lerp(_ColorDeep.rgb,_ColorShallow.rgb,pow(1.0-max(0,dot(normalDirection, viewDirection)),clamp(_ColorFresnel,0,4)))+node_4197));
                float specularMonochrome;
                float3 specColor;
