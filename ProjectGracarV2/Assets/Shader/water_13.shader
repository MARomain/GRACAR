    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 node_9286 = _Time;
                float2 node_4966 = (i.uv0+node_9286.g*float2(0.01,0.01));
                float4 _TextureNoise_var = tex2D(_TextureNoise,TRANSFORM_TEX(node_4966, _TextureNoise));
                float2 Refraction = (_TextureNoise_var.rgb.rg*_Refraction);
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 normalLocal = _normals;
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
                float4 node_1002 = _Time;
                float2 node_4966 = (i.uv0+node_1002.g*float2(0.01,0.01));
                float4 _TextureNoise_var = tex2D(_TextureNoise,TRANSFORM_TEX(node_4966, _TextureNoise));
                float2 Refraction = (_TextureNoise_var.rgb.rg*_Refraction);
                float2 sceneUVs = (i.projPos.xy / i.projPos.w) + Refraction;
                float4 sceneColor = tex2D(_GrabTexture, sceneUVs);
